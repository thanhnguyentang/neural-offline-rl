...
class LinPER(object):
    def __init__(self,
        ...
        self.reset()
    def evaluate(self):
        preds = np.zeros((self.M, self.num_test, self.bandit.n_arms))
        for m in range(self.M):
            # noise = np.array(
            #     [ np.random.multivariate_normal(mean=np.zeros(self.approximator_dim), \
            #         cov=self.perturbed_variance**2 * self.A_inv[a], size=self.num_test) \
            #         for a in self.bandit.arms ]
            #     ) # (a, n, d)
            # perturbed_theta_hat = self.theta_hat[:,None,:] + noise # (a,n,d)

            for a in tqdm(self.bandit.arms):
                perturbed_theta_hat = self.theta_hat[a,:][None, :] + np.random.multivariate_normal(mean=np.zeros(self.approximator_dim), \
                    cov=self.perturbed_variance**2 * self.A_inv[a], size=self.num_test)
                ...
                # convert cls 
                # preds[m,:,a] = np.sum(x_batch * perturbed_theta_hat[a,:,:], axis=1) # (n,)
                preds[m,:,a] = np.sum(x_batch * perturbed_theta_hat, axis=1) # (n,)
       
        # predicted_arms = np.argmax(np.min(preds, axis=0), axis=1).astype('int')
    ...
    def reset(self):
        ...
        """
        if self.is_cls_bandit:
            self.approximator_dim = self.bandit.features[0].shape[0] # self.bandit.n_arms * self.bandit.features[0].shape[0] 
        else: 
        ...
        with tqdm(total=self.T, postfix=postfix) as pbar:
            for t in range(self.T):
                ...
                if t % self.evaluate_every == 0: 
                    ...
                    print('\n[LinPER] t={}, subopt={}, % optimal arms={}, action select time={}'.format(t, self.subopts[-1][1], \
